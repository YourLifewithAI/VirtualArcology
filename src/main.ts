...
import { classifyBiome, fetchClimate, fetchElevationGrid, geocode } from './core/Location';
...
const locatePop = document.createElement('div');
locatePop.className = 'va-sitepop';
locatePop.style.display = 'none';
locatePop.style.left = '50%';
locatePop.style.top = '90px';
uiRoot.appendChild(locatePop);
...
interface SavedLocation {
  ...
  biome: string;
  climateLabel: string;
  heights?: { values: number[]; n: number; span: number };
}

function applySavedLocation(saved: SavedLocation): void {
  tessera.applyLocation({ label: `${saved.label} · ${saved.climateLabel}`, lat: saved.lat, biome: saved.biome, heights: saved.heights });
  localStorage.setItem('va-biome', saved.biome);
}

async function runLocate(query: string): Promise<void> {
  hud.showToast(`Looking up “${query}”…`);
  const loc = await geocode(query);
  if (!loc) {
    hud.showToast(`Couldn't find “${query}” — try a city name or "lat, lon"`);
    return;
  }
  hud.showToast(`${loc.label}: reading climate…`);
  const climate = await fetchClimate(loc.lat, loc.lon);
  const cls = classifyBiome(climate);
  hud.showToast(`${loc.label}: fetching real elevation…`);
  let heights: SavedLocation['heights'];
  try {
    const n = 21;
    const span = 6400;
    heights = { values: await fetchElevationGrid(loc.lat, loc.lon, n, span), n, span };
  } catch {
    heights = undefined; // climate/biome still applies; hills stay procedural
  }
  const saved: SavedLocation = { label: loc.label, lat: loc.lat, lon: loc.lon, biome: cls.biome, climateLabel: cls.label, heights };
  applySavedLocation(saved);
  try {
    localStorage.setItem('va-location', JSON.stringify(saved));
  } catch {
    /* heights can exceed quota in exotic cases — location still applied */
  }
  hud.showToast(
    `${loc.label} — ${cls.label} (${Math.round(climate.tMeanC)}°C · ${Math.round(climate.precipMm)} mm/yr)${heights ? ' · real terrain loaded' : ' · terrain unavailable, using hills'}`,
  );
}

function openLocateDialog(): void {
  locatePop.style.display = '';
  locatePop.innerHTML = `
    <b>Where is this Tessera?</b><br/>
    <span style="opacity:.7">City, address, or "lat, lon" — climate picks the region, real elevation shapes the land.</span><br/>
    <input class="va-locate-input" type="text" placeholder="e.g. near Seattle, WA" style="width: 240px; margin: 6px 0;"/>
    <br/>
    <button class="va-btn ok">Set location</button>
    <button class="va-btn clear">No location (hills)</button>
    <button class="va-btn cancel">Cancel</button><br/>
    <span style="opacity:.5; font-size: 10px">Geocoding © OpenStreetMap contributors (Nominatim) · climate &amp; elevation by Open-Meteo</span>`;
  const input = locatePop.querySelector('.va-locate-input') as HTMLInputElement;
  const close = (): void => {
    locatePop.style.display = 'none';
  };
  (locatePop.querySelector('.ok') as HTMLButtonElement).onclick = () => {
    const q = input.value.trim();
    close();
    if (q) {
      runLocate(q).catch((err) => hud.showToast(`Location lookup failed (offline?): ${String(err).slice(0, 60)}`));
    }
  };
  (locatePop.querySelector('.clear') as HTMLButtonElement).onclick = () => {
    close();
    localStorage.removeItem('va-location');
    tessera.clearLocation();
    hud.showToast('Placeless again — procedural hills');
  };
  (locatePop.querySelector('.cancel') as HTMLButtonElement).onclick = close;
  input.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') (locatePop.querySelector('.ok') as HTMLButtonElement).click();
    if (e.key === 'Escape') close();
  });
  input.focus();
}

// restore a saved location (no network needed — heights are cached)
try {
  const saved = JSON.parse(localStorage.getItem('va-location') ?? 'null') as SavedLocation | null;
  if (saved?.label && saved.biome) applySavedLocation(saved);
} catch {
  /* corrupted value — stay placeless */
}
